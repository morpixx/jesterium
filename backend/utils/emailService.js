...
async function sendVerificationCode(email, code) {
  const mailOptions = {
    ...
    to: email,
    subject: 'Jesterium - Підтвердження Email',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f9f9f9; padding: 20px; border-radius: 10px;">
        ...
        <div style="background-color: white; padding: 20px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
          <h2 style="color: #4a4a4a; text-align: center;">Підтвердження Email</h2>
          ...
          <p style="color: #666; line-height: 1.5;">Якщо ви не реєструвалися в Jesterium, просто проігноруйте цей лист.</p>
        </div>
        <div style="text-align: center; margin-top: 20px; color: #888; font-size: 12px;">
          <p>© ${new Date().getFullYear()} Jesterium. Всі права захищені.</p>
        </div>
      </div>
    `
  };
